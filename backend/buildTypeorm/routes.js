...
export async function doggr_routes(app) {
    ...
    app.post("/match", post_matches_opts, async (req, reply) => {
        const { matcherId, matcheeId } = req.body;
        //need to test
        try {
            let matcheeProfile = await app.db.profile.findOneOrFail({
                where: {
                    id: matcherId
                }
            });
            let matcherProfile = await app.db.profile.findOneOrFail({
                where: {
                    id: matcheeId
                }
            });
     ...
     */
    app.delete("/match/:matcherId/:matcheeId", async (req, reply) => {
        const { matcherId, matcheeId } = req.params;
        try {
     ...
     */
    app.delete("/matches/:matcherId", async (req, reply) => {
     ...
     */
    app.post("/message/:messageSender/:messageReceiver", async (req, reply) => {
        const { messageSender, messageReceiver } = req.params;
        const { message } = req.body;
        let senderProfile = await app.db.profile.findOneOrFail({
            where: {
                id: messageSender
            }
        });
        let receiverProfile = await app.db.profile.findOneOrFail({
            where: {
                id: messageReceiver
            }
        });
